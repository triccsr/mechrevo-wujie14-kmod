...
#if !defined (NO_PLATFORM_PROFILE)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 14, 0)
static int wujie14_platform_profile_probe(void* drvdata, unsigned long* choices)
{
...
static const struct platform_profile_ops wujie14_platform_profile_ops = {
...
int wujie14_platform_profile_init(struct wujie14_private* priv)
{
    ...
    priv->ppdev = platform_profile_register(
        ...
        "wujie14",
        priv,
        &wujie14_platform_profile_ops);
    if (IS_ERR(priv->ppdev)) {
        err = PTR_ERR(priv->ppdev);
        priv->ppdev = NULL;
    }
#else
